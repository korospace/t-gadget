<nav class="w-full fixed top-0 text-tgadget" style="z-index:10001;">
    
    <div class="bg-black w-full relative flex justify-between items-center pl-5 md-911:pl-10 pr-5 md-911:pr-10 py-5" style="z-index:10000;">
        <!-- logo -->
        ...
        <!-- burger-kategori -->
        <span id="burger-kategori" class="hidden sm-411:inline ml-4 sm:ml-6 md:ml-8 mr-2 md:mr-4 px-2 py-1 text-xs md:text-sm rounded-sm cursor-pointer opacity-90 hover:opacity-100">kategori</span>

        <div class="relative flex-1 overflow-visible" style="color: rgba(49,53,59,0.96);">
            <!-- input keyword -->
            <div class="bg-white relative border-2 border-tgadget rounded-sm overflow-hidden">
                ...
                <label for="input-keyword" class="bg-tgadget w-8 sm:w-10 absolute top-0 bottom-0 right-0 block p-0.5 sm:p-0 cursor-pointer">
                    <img class="w-full h-full p-1 opacity-80 hover:opacity-100" src="<?= BASE_URL; ?>asset/img/search.svg">
                </label>
            </div>

            <!-- keywords sugestion -->
            <ul id="keywords-sugestion" class="bg-white w-full absolute block mt-1 text-xs sm:text-sm rounded-sm overflow-auto" style="max-height: 200px;box-shadow: 2px 2px 6px 0px rgba(0,0,0,0.3);"></ul>
        </div>
    ...
    <!-- Categories -->
    <div id="kategori-wraper" class="bg-black w-full h-0 transition-all duration-300 overflow-auto overflow-hidden" style="border-color: rgba(255, 185, 3,0.3);">
        <div class="min-w-max h-full flex hidden px-5 mymd:px-10 text-myxs sm:text-xs md:text-sm capitalize tracking-wide transition-all duration-300 opacity-0">
    ...
    <div id="footer" class="bg-footer relative pt-10 px-5 mymd:px-10 w-full">
        ...
        <div class="w-full flex flex-col mysm:flex-row justify-between items-center">
            <div>
                ...
                <div class="mt-4 flex text-tgadget text-md mysm:text-xs sm:text-sm md:text-base tracking-wide">
            ...
            <a class="contact-link bg-black px-2.5 sm:px-4 py-1.5 sm:py-2 hidden mysm:inline border-2 border-tgadget text-tgadget text-xs sm:text-sm md:text-base tracking-widest opacity-80 hover:opacity-100 rounded-sm" style="box-shadow: 2px 2px 6px 0px  rgba(0,0,0,0.3);"  data-atribut="whatsapp" data-href="https://mywa.link/3xwr4ga2" href="">contact us</a>
        </div>
        <div class="mt-4 w-full py-8 flex flex-col justify-between items-center" style="border-top: 0.8px solid rgba(255, 185, 3,0.5);">
            <h1 class="text-xs sm:text-sm text-tgadget opacity-80 tracking-wide">Copyright © 2020-<?= date("Y"); ?> T-GadgetID</h1>
...
<div id="popup-container" class="bg-testi-clicked close px-16 hidden z-min1 fixed top-0 bottom-0 left-0 right-0 flex justify-center items-center" onclick="closeCountDown(this,event);">
    <div id="popup-wraper" class="relative w-72">
        <div id="close-popup" class="bg-tgadget close absolute z-30 -top-2 mysm:-top-3 -right-2 mysm:-right-3 w-6 mysm:w-8 p-2 rounded-full cursor-pointer" onclick="closeCountDown(this,event);">
            <img class="w-full close" src="<?= BASE_URL; ?>asset/img/cancel.svg">
        </div>
        <div id="bg-img-popup" class="bg-tgadget p-0.5 w-full rounded-tl-xl rounded-tr-xl rounded-b-md">
        ...
        <div id="bg-countdowb" class="bg-tgadget mt-2 w-full p-0.5 rounded-md">
...
<div id="detail-container" class="bg-testi-clicked hidden z-min1 pt-16 fixed top-0 bottom-0 left-0 right-0 flex flex-col justify-end" style="backdrop-filter: blur(4px);-webkit-backdrop-filter: blur(8px);">
    <!-- detail wraper -->
    <div id="detail-wraper" class="bg-gradient-to-r from-yellow-500 to-tgadget relative z-20 w-full h-0 flex flex-col sm:flex-row rounded-tl-xl rounded-tr-xl transition-all duration-300 overflow-hidden">
        <!-- close -->
        ...
        <div id="deskription-wraper1" class="pt-0 sm:pt-10 md:pt-14 fixed sm:static bottom-0 w-full sm:w-auto sm:flex-1 h-0 sm:h-full opacity-0 transition-all duration-300 overflow-hidden">
            <div id="deskription-wraper2" class="bg-gradient-to-r from-yellow-500 sm:from-transparent to-tgadget sm:to-transparent w-full h-full rounded-t-xl sm:rounded-t-none overflow-hidden">
                ...
                <div id="deskription-wraper3" class="pr-5 mymd:pr-10 pl-5 sm:pl-3 mymd:pl-0 w-full h-full overflow-auto">
                    <div id="bag-1" class="w-full h-auto mb-3 mysm:mb-4 ">
                        <h1 id="status" class="bg-black w-max text-tgadget text-xs lg:text-sm tracking-wide px-2 py-1">lorem</h1>
    ...
    <div id="buy-container" class="bg-testi-clicked fixed z-min1 top-0 bottom-0 left-0 right-0 close flex justify-center items-center opacity-0" onclick="closeBuy(this,event);">
        <div id="buy-wraper1" class="bg-gradient-to-r from-yellow-500 to-tgadget mx-3 w-24 p-1 rounded-md transition-all duration-700 delay-100 opacity-0">
            <div id="buy-wraper2" class="p-2 w-full h-full border-2 border-black rounded-md flex flex-col items-center">
                ...
                <div class="w-full flex mt-8">
                    <div class="bg-black flex-1 flex items-center rounded-md px-2 py-2.5 overflow-hidden">
                        ...
                        <div id="link-tokped" class="link text-tgadget text-xs" style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;overflow: hidden;text-overflow: ellipsis;"></div>
                    </div>
                ...
                <div class="w-full flex mt-3">
                    <div class="bg-black flex-1 flex items-center rounded-md px-2 py-2.5 overflow-hidden">
                        ...
                        <div id="link-shopee" class="link text-tgadget text-xs" style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;overflow: hidden;text-overflow: ellipsis;"></div>
                    </div>
                ...
                <div class="w-full flex mt-3">
                    <div class="bg-black flex-1 flex items-center rounded-md px-2 py-2.5 overflow-hidden">
                        ...
                        <div id="link-lazada" class="link text-tgadget text-xs" style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;overflow: hidden;text-overflow: ellipsis;"></div>
                    </div>
                ...
                <div class="w-full flex mt-3">
                    <div class="bg-black flex-1 flex items-center rounded-md px-2 py-2.5 overflow-hidden">
                        ...
                        <div id="link-wa" class="link text-tgadget text-xs" style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;overflow: hidden;text-overflow: ellipsis;"></div>
                    </div>
